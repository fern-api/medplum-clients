...
final class ImmutableAccessPolicy implements AccessPolicy {
  ...
  /**
   * Computes a hash code from attributes: {@code implicitRules}, {@code compartment}, {@code resource}, {@code id}, {@code name}, {@code resourceType}, {@code language}, {@code meta}.
   ...
   *    .implicitRules(com.medplum.types.fhir.Uri) // optional {@link AccessPolicy#implicitRules() implicitRules}
   *    .compartment(com.medplum.types.fhir.Reference) // optional {@link AccessPolicy#compartment() compartment}
   *    .resource(List&amp;lt;com.medplum.types.fhir.AccessPolicy_Resource&amp;gt;) // optional {@link AccessPolicy#resource() resource}
   *    .id(com.medplum.types.fhir.Id) // optional {@link AccessPolicy#id() id}
   *    .name(String) // optional {@link AccessPolicy#name() name}
   *    .resourceType(String) // optional {@link AccessPolicy#resourceType() resourceType}
   *    .language(com.medplum.types.fhir.Code) // optional {@link AccessPolicy#language() language}
   *    .meta(com.medplum.types.fhir.Meta) // optional {@link AccessPolicy#meta() meta}
   *    .build();
   * </pre>
  ...
  public static final class Builder {
    ...
    private static final long OPT_BIT_RESOURCE_TYPE = 0x20L;
    ...
    public AccessPolicy build() {
      return new ImmutableAccessPolicy(implicitRules, compartment, resource, id, name, resourceType, language, meta);
    }
